...
const Chart = (props) => {
    const PriceOnlyArray = props.chartExpenses.map((monthExpenseData) => {
        return monthExpenseData.price;
    })
    const MaxValue = Math.max(...PriceOnlyArray);
    ...
    //     chartData[props.chartExpenses[i].date.getMonth()].expenseValue += parseFloat(props.chartExpenses[i].price);
    // }
